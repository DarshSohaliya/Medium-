...
export const BlogCard = ({
    ...
    <div className=" p-4 border-b border-slate-200 pb-4 w-screen max-w-screen-md cursor-pointer">
        ...
        <div className="text-thin text--md">
            {content.slice(0,100) + "..."}
        </div>
        <div className="text-slate-500 font-thin text-sm pt-2">
...
export function Avatar({name ,size = "small"} : {name :string , size: "small" | "big"}){
   ...
   <span className={`${size === "small" ? "text-xs" : "text-md"} font-semibold text-white`}>{name[0]}</span>
